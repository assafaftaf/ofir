...
interface EditCadetScreenProps {
  cadet: Cadet
  onBack: () => void
}

export default function EditCadetScreen({ cadet, onBack }: EditCadetScreenProps) {
  const [formData, setFormData] = useState(cadet)
  const { toast } = useToast()
  const { updateCadet } = useCadetContext()
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (Object.values(formData).every((value) => value)) {
      ...
      toast({
        title: "פרטי הצוער עודכנו בהצלחה!",
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8 text-center">עריכת פרטי צוער</h1>
